(function ($) {
    ...
    var methods = {
        init: function (options) {
            ...
            return this.each(function () {
                ...
                if (!list.name) {
                    throw 'jQuery.list requires a name to be specified';
                }
                if (!$.isArray(list.headers)) {
                    throw 'jQuery.list requires headers to be specified, must be an array';
                }
                if (list.headers.length
                    < 1) {
                    throw 'jQuery.list headers must contain at least one item, header can be a string or an object { field : string, label : string, sortable : boolean (default : false), escape : boolean (default : true), format : forgetrmatting function }';
                }
                if (!$.isFunction(list.source)) {
                    throw 'jQuery.list requires a source callback';
                }

                if (list.group
                    && !$.isPlainObject(list.group)) {
                    throw 'jQuery.list group option must be an object { field : "field_name", groupClass : "css-class", reset : boolean}';
                }
        ...
        options: function () {
            ...
            if (arguments.length === 0 || (arguments.length == 1 && typeof arguments[0] == 'string')) {
                ...
                return this.each(function () {
                    var list = $(this).data('list');
                    if (list) {
                        list.setOptions.apply(list, args);
                    }
                });
            }
        },
        ...
        update: function (type, params) {
            ...
            return this.each(function () {
                var list = $(this).data('list');
                if (list) {
                    list.update[type].call(list, params);
                }
            });
        },
        ...
        sort: function (header, asc, update) {
            if (arguments.length === 0) {
                ...
                return this.each(function () {
                    ...
                    if (list) {
                        list.sortHeader(header, asc, update);
                    }
                });
            }
        },
        ...
        page: function (page, update) {
            if (arguments.length === 0) {
                ...
                return this.each(function () {
                    ...
                    if (list) {
                        list.setPage(page, list.nbPages, update);
                    }
                });
            }
        },
        ...
        setContextValue: function (key, value, update) {
            return this.each(function () {
                ...
                if (list) {
                    list.setContextValue(key, value, update);
                }
            });
        },
        ...
        updateOddClasses: function () {
            return this.each(function () {
                ...
                if (!list.data || !$.isArray(list.data) || list.data.length === 0) {
                    return;
                }
                ...
                $all_rows.each(function (i, elem) {
                    var $this = $(this);
                    if (list.oddClass) {
                        $this.removeClass(list.oddClass);
                    }
                    if (list.evenClass) {
                        $this.removeClass(list.evenClass);
                    }
                    if (i % 2 !== 0 && list.oddClass) {
                        $this.addClass(list.oddClass);
                    } else if (i % 2 === 0 && list.evenClass) {
                        $this.addClass(list.evenClass);
                    }
                });
            });
        },
    ...
    $.fn.list = function (method) {
        ...
        if (!$.app) {
            throw 'jQuery.list cannot be used outside kronos-libs framework';
        }
    ...
    $.fn.list.defaults = {
        ...
        buildTable: function () {
            ...
            html += '</tr></thead><tbody></tbody></table><div id = "' + this.name
                + '_pagination" class="pagination-wrapper"></div>';
                ...
                if ($.isPlainObject(this.headers[i])) {
                    ...
                    if (this.headers[i].sortable === true) {
                        var lastSpace = content.lastIndexOf(' '),
                            ...
                            after = '<span class="nowrap">' + content.substring(lastSpace)
                    ...
                    if (this.sort == this.headers[i].sortField) {
                        ...
                        if (currentHeader.hasClass('sorted')) {
                            currentHeader.removeClass('sorted');
                        }
                    }
        ...
        fetchData: function () {
            ...
            if (this.page === 0 && this.pagination == 'more') {
                this.clear();
            }
            ...
            if (!this.data || !$.isArray(this.data) || this.data.length === 0) {
                ...
                for (var i = 0; i < this.data.length; i++) {
                    ...
                    if ($.isFunction(this.click)) {
                        $('#' + this.name + '_row_' + current_i).data('index', current_i).data('record', this.data[i])
                            .safeClick(doClick);
                    }
                }
            }
        ...
        drawRow: function (record, index, odd) {
            ...
            for (var i = 0; i < this.headers.length; i++) {
                ...
                var td = $('<td></td>').attr('id', this.name + '_row_' + index + '_' + i).data('index', index)
                    .data('i', i);
        ...
        noData: function () {
            return '<tr class="non-clickable-row' + (this.evenClass ? ' ' + $.app.htmlEntity(this.evenClass) : '')
                + '"><td colspan=' + this.headers.length + '">' + $.app._('NO_RESULTS') + '</td></tr>';
        },
        ...
        setPage: function (page, nbPages, update) {
            if (this.page == page && this.nbPages == nbPages) {
                return false;
            }
        ...
        setOptions: function () {
            ...
            if (page_changed && this.pagination == 'pages') {
                this.updatePagination.pages.call(this);
            }
            if (hasMore_changed) {
                this.updatePagination.more.call(this);
