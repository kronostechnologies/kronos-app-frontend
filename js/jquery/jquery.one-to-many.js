    ...
    var methods = {
        ...
        injectOnExistingRow: function (injectObj, $row) {
            for (var i = 0; i < injectObj.length; i++) {
                ...
                if (typeof injectObj[i].before === 'function') {
                    injectObj[i].before($row);
                }

                $form_elem.val(injectObj[i].val);

                if (typeof injectObj[i].after === 'function') {
                    injectObj[i].after($row);
                }
            }
        },
        ...
        delete_row: function ($row) {
            ...
            if ($row.siblings('.mto-row').length === 0) {
                ...
                var eventParams = ['mto'];
                $row.find('input, textarea').not('.no-clear').val('').trigger('change', eventParams)
                    .trigger('blur', eventParams);
                $row.find('select:not(.no-clear)').find('option:first').attr('selected', 'selected')
                ...
                if ($row.next().length) {
                    $row.next().find('input:tabbable,select:tabbable,textarea:tabbable')
                        .not('.mto-nofocus').first().focus();
                } else if ($row.prev().length) {
                    $row.prev().find('input:tabbable,select:tabbable,textarea:tabbable')
                        .not('.mto-nofocus').first().focus();
                }
                $row.remove();
                methods.update_altrow.call(this);
