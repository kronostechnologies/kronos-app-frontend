export default (jQuery, app) => ({
    ...
    durationVal(months_duration) {
        ...
        if (!isFinite(months_duration)) {
            ...
            text = sign + (years === 0 ? '' : `${years === 1 ? app._('YEARD').replace('%{years}', years) : app._(
                'YEARSD',
            ).replace('%{years}', years)} `)
                + (months === 0 ? '' : (months === 1 ? app._('MONTHD').replace('%{months}', months) : app._('MONTHSD')
            ...
            jQuery(this).blur(function () {
                ...
                if (opts && opts.canBeNull && (value === '' || value === null)) {
                    ...
                    if (opts && opts.true_precision) {
                        if (opts.rounded) {
                            val = Math.round(val * Math.pow(10, opts.true_precision)) / Math.pow(10,
                                opts.true_precision);
                        } else {
                            val = Math.floor(val * Math.pow(10, opts.true_precision)) / Math.pow(10,
                                opts.true_precision);
                        }
                    }
                }
    ...
    datepickerInput(interval) {
        return jQuery(this).each(function () {
            ...
            if ($this.next('.btn.calendar').length === 0) {
                $calendar_button = $(
                    '<a href="javascript:void(0);" class="btn calendar" tabindex=-1><span class="icon"></span></a>',
                );
            }
    ...
    hint(text, width) {
        ...
        return jQuery(this)
            ...
            .after(`<span class="hint" style="width: ${w}px; right: -${w
            + 50}px;">${text}<span class="hint-pointer sprite">&nbsp;</span></span>`)
            .focus(function () {
                const t = this;
